...
// Baole Ai 2016 add for full_label
...
void DataHeatmapLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    /////////////////////////////////////////////////ai201604/////////////////////////////////////////////////////
    //add top[2] for full_label_
#ifdef FLIC
    top[2]->Reshape(label_batchsize, 4, label_height, label_width);
    for (int i = 0; i < this->PREFETCH_COUNT; ++i)
    	this->prefetch_[i].limb_label_.Reshape(label_batchsize, 4, label_height, label_width);
#else
    top[2]->Reshape(label_batchsize, 14, label_height, label_width);
    for (int i = 0; i < this->PREFETCH_COUNT; ++i)
    	this->prefetch_[i].limb_label_.Reshape(label_batchsize, 14, label_height, label_width);
#endif
    ...
    LOG(INFO) << "output limb label size" << top[2]->num() << "," << top[2]->channels() << "," << top[2]->height() << "," << top[2]->width();
}
...
void DataHeatmapLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    //////////////////////////////////////////////ai201604//////////////////////////////////////////////////////////
    Dtype* top_limb_label = batch->limb_label_.mutable_cpu_data();
    //////////////////////////////////////////////ai201604/////////////////////////////////////////////////////////
    ...
    const float sigma = heatmap_data_param.sigma();
    ...
    int flip_start_ind;
    //////////////////////////////////////////////////////////////change for lsp///////////////////////////////////////
    // lsp is 4 and flic is 2.
#ifdef FLIC
    if (dont_flip_first) flip_start_ind = 2;
    else flip_start_ind = 0;
#else
    if (dont_flip_first) flip_start_ind = 4;
    else flip_start_ind = 0;
#endif
    ...
    for (int idx_img = 0; idx_img < batchsize; idx_img++)
    {
        ...
        for (int idx_aug = 0; idx_aug < num_aug; idx_aug++)
        {
            ...
            {
            	// for lsp
                //if (idx_ch == 6 || idx_ch == 7 || idx_ch == 12 || idx_ch == 13)
                	//sigma_ = 2 * sigma;
                //else
                	//sigma_ = sigma;
                ...
                for (int i = 0; i < label_height; i++)
                {
                    for (int j = 0; j < label_width; j++)
                    {
                        ...
                        float gaussian = exp( -0.5 * ( pow(i - y, 2.0) + pow(j - x, 2.0) ) * pow(1 / sigma_, 2.0) );
                        //gaussian = 4 * gaussian;( 1 / ( sigma * sqrt(2 * M_PI) ) ) * 
...
#if 1
#ifdef FLIC
            const int label_limb_size = label_channel_size * 4;
            //////////////////////////////////////////////////////////////
            // upper limb 0 1 2 3
            // for lsp 2-6 idx_ch -2
            // for flic 1-5 idx_ch -1
            for (int idx_ch = 1; idx_ch < 5; idx_ch++)
            {
                float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+2)] * multfact;
                float y2 = label_height - label_resize_fact * cur_label_aug[2 * (idx_ch+2) + 1] * multfact;
                float X1, X2, Y1,Y2;
                ...
                if ((x1==x2)&&(y1==y2))
                {
                	for (int i = 0; i < label_height; i++)
                	{
                		int i_ = label_height - i;
                		for (int j = 0; j < label_width; j++)
                		{
                			int label_idx = idx_img_aug * label_limb_size + (idx_ch-1) * label_channel_size + i * label_height + j;
                			float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                			top_limb_label[label_idx] = gaussian;
                		}
                	}
                }
                else if ((x1==x2)&&(y1!=y2))
                {
                	...
                	for (int i = 0; i <= label_height; i++)
                	{
                		...
                		for (int j = 0; j < label_width; j++)
                		{
                			...
                			if((i_<=Y1)&&(i_>=Y2))
                			{
                				float gaussian = exp( -0.5 * (pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                				top_limb_label[label_idx] = gaussian;
                			}
                			else
                				top_limb_label[label_idx] = 0;
                ...
                else if ((y1==y2)&&(x1!=x2))
                {
                	...
                	for (int i = 0; i < label_height; i++)
                	{
                		...
                		for (int j = 0; j <= label_width; j++)
                		{
                			...
                			if ((j<=X1)&&(j>=X2))
                			{
                				float gaussian = exp( -0.5 * (pow(i_ - y1, 2.0) ) * pow(1 / sigma, 2.0) );
                				top_limb_label[label_idx] = gaussian;
                			}
                			else
                				top_limb_label[label_idx] = 0;
                ...
                {
                	if (y1 > y2){Y1 = y1; X1 = x1;Y2 = y2; X2 = x2;}
                	else {Y1 = y2; X1 = x2; Y2=y1; X2 = x1;}
                	float alpha = (Y1-Y2)/(X1-X2);
                	float alpha_ = -1/alpha;
                	float b1 = Y1 - alpha_ * X1;
                	float b2 = Y2 - alpha_ * X2;
                	float A = Y2 - Y1;
                	float B = X1 - X2;
                	float C = X2*Y1 - X1*Y2;
                	for (int i = 0; i < label_height; i++)
                	{
                		...
                		{
                			int label_idx = idx_img_aug * label_limb_size + (idx_ch-1) * label_channel_size + i * label_height + j;
                			if ((i_ <= alpha_*j + b1)&&(i_ >= alpha_*j + b2))
							{
                				float gaussian = exp( -0.5 * (pow((A * j + B * i_ + C), 2.0) )/(pow(A,2.0) + pow(B,2.0)) * pow(1 / sigma, 2.0) );
                				top_limb_label[label_idx] = gaussian;
							}
                			else
                				top_limb_label[label_idx] = 0;
...
#else
            ////////////////////////////////////////////////ai201604////////////////////////////////////////////////////////////////////////////////////////
            // add for top_limb_label
            // store limb label
            //for lsp
            //const int label_limb_size = label_channel_size * 14;
            const int label_limb_size = label_channel_size * 14;
            //////////////////////////////////////////////////////////////
            // upper limb 0 1 2 3
            // for lsp 2-6 idx_ch -2
            // for flic 1-5 idx_ch -1
            for (int idx_ch = 2; idx_ch < 6; idx_ch++)
            {
                float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+2)] * multfact;
                ...
                if ((x1==x2)&&(y1==y2))
                {
                	for (int i = 0; i < label_height; i++)
                	{
                		...
                		for (int j = 0; j < label_width; j++)
                		{
                			int label_idx = idx_img_aug * label_limb_size + (idx_ch-2) * label_channel_size + i * label_height + j;
                			float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                			top_limb_label[label_idx] = gaussian;
                		}
                	}
            ...
            //////////////////////////////////////////////////////////
            // rshoulder -neck 4
            for(int idx_ch = 1 ; idx_ch < 2; idx_ch++)
            {
            	float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+1)] * multfact;
                float y2 = label_height - label_resize_fact * cur_label_aug[2 * (idx_ch+1) + 1] * multfact;
                float X1, X2, Y1,Y2;
                ...
                if ((x1==x2)&&(y1==y2))
                {
                	for (int i = 0; i < label_height; i++)
                    {
                        ...
                        for (int j = 0; j < label_width; j++)
                         {
                        	int label_idx = idx_img_aug * label_limb_size + (idx_ch+3) * label_channel_size + i * label_height + j;
                            float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                            top_limb_label[label_idx] = gaussian;
                          }
                     }
            ...
            for(int idx_ch = 1 ; idx_ch < 2; idx_ch++)
            {
                ...
                if ((x1==x2)&&(y1==y2))
                {
                	for (int i = 0; i < label_height; i++)
                    {
                        ...
                        for (int j = 0; j < label_width; j++)
                         {
                        	int label_idx = idx_img_aug * label_limb_size + (idx_ch+4) * label_channel_size + i * label_height + j;
                            float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                            top_limb_label[label_idx] = gaussian;
                          }
                     }
            ...
            //////////////////////////////////////////////////////////
            //head -- neck 6
                        for(int idx_ch = 0 ; idx_ch < 1; idx_ch++)
                        {
                        	float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                            float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                            float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+1)] * multfact;
                            ...
                            if ((x1==x2)&&(y1==y2))
                            {
                            	for (int i = 0; i < label_height; i++)
                                {
                                    ...
                                    for (int j = 0; j < label_width; j++)
                                     {
                                    	int label_idx = idx_img_aug * label_limb_size + (idx_ch+6) * label_channel_size + i * label_height + j;
                                        float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                                        top_limb_label[label_idx] = gaussian;
                                      }
                                 }
                        ...
                        //////////////////////////////////////////////////////////////
                        //lshoulder--lhip rshoulder--rhip 7 8
                        for (int idx_ch = 2; idx_ch < 4; idx_ch++)
                        {
                            float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                            float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                            float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+6)] * multfact;
                            float y2 = label_height - label_resize_fact * cur_label_aug[2 * (idx_ch+6) + 1] * multfact;
                            float X1, X2, Y1,Y2;
                            ...
                            if ((x1==x2)&&(y1==y2))
                            {
                            	for (int i = 0; i < label_height; i++)
                            	{
                            		...
                            		for (int j = 0; j < label_width; j++)
                            		{
                            			int label_idx = idx_img_aug * label_limb_size + (idx_ch+5) * label_channel_size + i * label_height + j;
                            			float gaussian = exp( -0.5 * ( pow(i_ - y1, 2.0) + pow(j - x1, 2.0) ) * pow(1 / sigma, 2.0) );
                            			top_limb_label[label_idx] = gaussian;
                            		}
                            	}
                        ...
                        //////////////////////////////////////////////////////////
                        //lhip--rhip 9
                        for(int idx_ch = 8 ; idx_ch < 9; idx_ch++)
                        {
                        	float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                            float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                            float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+1)] * multfact;
                        ...
                        //////////////////////////////////////////////////////////
                        // lower limb 10 11 12 13
                        for (int idx_ch = 8; idx_ch < 12; idx_ch++)
                        {
                            float x1 = label_resize_fact * cur_label_aug[2 * idx_ch] * multfact;
                            float y1 = label_height - label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
                            float x2 = label_resize_fact * cur_label_aug[2 * (idx_ch+2)] * multfact;
...
//	    cv::line(img_annotation_vis, centers[2], centers[3], CV_RGB(255, 255, 0), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[2], centers[4], CV_RGB(0, 0, 255), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[3], centers[5], CV_RGB(0, 255, 255), 1, CV_AA);
//	    // add for new joints 14
//	    cv::line(img_annotation_vis, centers[4], centers[6], CV_RGB(0, 128, 0), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[5], centers[7], CV_RGB(0, 128, 255), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[8], centers[10], CV_RGB(128, 128, 0), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[9], centers[11], CV_RGB(128, 255, 0), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[10], centers[12], CV_RGB(0, 0, 128), 1, CV_AA);
//	    cv::line(img_annotation_vis, centers[11], centers[13], CV_RGB(0, 255, 128), 1, CV_AA);
////////////////////////////////////////////baole AI@201604//////////////////////////////////////////////////////
}
